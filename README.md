...
1. Логін пише `jwt_token` + `refresh_token` у secure storage. Пароль не зберігається.
2. Access живе 15 хв, refresh — 7 днів. На старті валідний access → сесія; протухлий access → `/login`, refresh лишається для Face ID / Touch ID.
3. На логіні, якщо є refresh і біометрія пристрою, кнопка входу оновлює токени через `/auth/refresh`. Пароль потрібен після logout або коли refresh сплив.
4. Access додається в кожен REST-запит і в заголовки `Image.network` для захищених файлів.
5. Після паузи ≥ 2 хв (або cold start з живою сесією) показується `AppLockOverlay`.
6. Розблокування живої сесії — біометрія або код пристрою (`biometricOnly: false`). Якщо не виходить — «Вийти з акаунта».
